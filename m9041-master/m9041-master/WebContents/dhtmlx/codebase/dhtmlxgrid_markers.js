...
dhtmlXGridObject.prototype.doMark=function(a,b){var c=a.parentNode.idd,d=a._cellIndex;if (!c) return; b||(b=0);if(b==0)this.unmarkAll();else if(b==1){if(this.lastMarkedRow)for(var h=Math.min(this.getRowIndex(c),this.getRowIndex(this.lastMarkedRow)),i=Math.max(this.getRowIndex(c),this.getRowIndex(this.lastMarkedRow)),j=Math.min(d,this.lastMarkedColumn),k=Math.max(d,this.lastMarkedColumn),e=h;e<i+1;e++)for(var f=j;f<k+1;f++)this.mark(this.getRowId(e),f,!0)}else if(b==2&&this.markedRowsArr._dhx_find(c)!=-1)for(var g=
